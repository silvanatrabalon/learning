...
## Pipelines CI/CD - GitHub Actions y Jenkins
**Descripción:** Automatización de procesos de integración continua y despliegue continuo usando GitHub Actions y Jenkins, incluyendo workflows, jobs, stages y mejores prácticas para pipelines robustos.
...
**Comparación:** GitHub Actions vs Jenkins - GitHub Actions es nativo para repos GitHub con sintaxis YAML simple, mientras que Jenkins es más potente y flexible con Groovy, mejor para pipelines complejos y on-premise.

## Containers - Docker y Kubernetes
**Descripción:** Containerización de aplicaciones con Docker y orquestación con Kubernetes, incluyendo Dockerfiles optimizados, Docker Compose, registries y deployments en K8s con Helm.
...
**Comparación:** Docker vs VMs - Docker es más liviano y portable usando kernel compartido, mientras que VMs aíslan completamente el OS. Kubernetes vs Docker Compose - K8s es para producción con alta disponibilidad, Compose es para desarrollo local.

## IaaS y Infrastructure as Code
**Descripción:** Gestión de infraestructura cloud con Terraform, CloudFormation y Pulumi, incluyendo AWS/GCP/Azure services, estado remoto, módulos reutilizables y mejores prácticas de IaC.

**Ejemplo:**
```hcl
# Terraform - Infraestructura AWS completa
terraform {
...
**Comparación:** Terraform vs CloudFormation vs Pulumi - Terraform es multi-cloud con HCL declarativo, CloudFormation es específico AWS con YAML/JSON, mientras que Pulumi usa lenguajes de programación reales para mayor flexibilidad.

## Environments y Deployment Strategies
**Descripción:** Gestión de múltiples entornos (dev, staging, prod) con estrategias de despliegue como Blue/Green, Canary releases, Feature Flags y técnicas para minimizar downtime y riesgo.
...
**Comparación:** Blue/Green vs Canary vs Rolling - Blue/Green cambia todo el tráfico instantáneamente con rollback rápido, Canary despliega gradualmente monitoreando métricas, mientras que Rolling actualiza instancias progresivamente sin downtime pero sin control granular del tráfico.

## Monitoreo y Observabilidad
**Descripción:** Implementación de sistemas de monitoreo completos con métricas, logs, trazas y alerting usando Grafana, Prometheus, New Relic, DataDog, ELK Stack y herramientas de APM para observabilidad total.
...
**Comparación:** Prometheus vs New Relic vs DataDog - Prometheus es open-source con pull-based metrics, New Relic/DataDog son SaaS con APM completo y mayor facilidad de setup, mientras que ELK Stack es ideal para logs centralizados y análisis de texto.

## Herramientas y Automatización DevOps
**Descripción:** Conjunto de herramientas esenciales para automatización DevOps incluyendo Ansible, Vault, Consul, service discovery, configuration management y orquestación de infraestructura.

**Ejemplo:**
```yaml
# Ansible Playbook completo
# site.yml
---
...
**Comparación:** Ansible vs Terraform vs Pulumi - Ansible es imperativo para configuración y orquestación, Terraform es declarativo para infraestructura, Pulumi combina infraestructura con código programático. Vault vs Consul - Vault se enfoca en gestión de secretos, Consul en service discovery y configuración distribuida.
